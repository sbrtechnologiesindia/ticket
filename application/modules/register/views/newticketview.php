         ...
         <div class="child_cake">
            <div class="drop_down_pi">
              ...
              if($ratingmod==1)
              {
                ...
                {
                  ?>
                  <strong style="float:left;font-size: 15px;padding-right: 6px;">Rating : </strong><div class="basic jDisabled" data-average="<?php echo $ticketdetlist->rating;?>" data-id="<?= $ticketdetlist->id ?>"></div>
                  <?php
                }
              }
              ?>
...
<!-- reply section -->
 ...
 <div class="sec_act">
        ...
        <p class="time"><?= date('ga',strtotime($ticketdetlist->create_date)) ?></p>
          ...
          <div class="right_rec">
             <?php 
               $userinfo = getUserInfoById($ticketdetlist->userid);
               $name = $userinfo->firstname.' '.$userinfo->lastname;
            ?>
            <p class="icon_top">
                        <img width="45" height="45"  src="<?= getGravatar($userinfo->email) ?>">
                       </p>
                        <div class="corner">
                      </div>
         ...
         <div class="sec_act">
          <div class="porfile_box">
        <p class="date"><?= date('d',strtotime($query->create_date)) ?>
        <br>
        <span><?= date('M',strtotime($query->create_date)) ?></span>
        </p>
        <p class="time"><?= date('ga',strtotime($query->create_date)) ?></p>
       </div>
         
          <div class="right_rec">
                       <?php 
               $userinfo = getUserInfoById($query->replier_id);
               $name = $userinfo->firstname.' '.$userinfo->lastname;
                        ?>
            <p class="icon_top">
                        <img width="45" height="45"  src="<?= getGravatar($userinfo->email) ?>">
