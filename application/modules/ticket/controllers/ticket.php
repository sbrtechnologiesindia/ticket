...
class ticket extends MX_Controller {
		...
		if(!empty($flag) or !empty($chatflag))
		{
			...
			if(!empty($attachments))
			{
				foreach($attachments as $attachment)
				{
					...
					$ext = end($arr);
					$filename = create_guid().".".$ext; 
					$sessionid=$this->session->userdata['session_id'];
					$txt = "";
						$scan = OPSWAT_scan_file(TICKET_PLUGIN_PATH."/tmp/".$attachment);
						if(!empty($scan))
						{
							$report = OPSWAT_scan_report($scan['rest_ip'],$scan['data_id']);
	...
	{
		@session_start();
		$arr=getSettingsValue('ticket_attachment');
		...
		if($ticke_post_type)
		{
			...
			if(!empty($_FILES['fileupload']))
			{
				...
				if(!empty($filedet))
				{
					foreach($filedet as &$file)
					{
						...
						if($chkopswat==1)
						{
							$scan = OPSWAT_scan_file(TICKET_PLUGIN_PATH."/CI/assets/attachments/".$file['filename']);
							if(!empty($scan))
							{
								$report = OPSWAT_scan_report($scan['rest_ip'],$scan['data_id']);
			...
			$userdet=getUserInfoById($userid);
			...
			if(!empty($ins))
			{
				...
				sendmail($adminemail,"","","Ticketing System : New Ticket Posted (".$ticket_name."): ",$messagebody,"info@ticketingsystem.com");
				...
				{
					$responsetime=smarttimestamp($responsetime);
				}
				...
				if($checkset==1)
				{
					...
					sendmail($useremail,$firstname,$lastname,"Auto Responder for Ticket Number ".$ticket_name." : ",$messagebodyticket,"info@ticketingsystem.com");
				}
				//code for auto responder
		...
		else
		{
			...
			if(!empty($ins))
			{
				...
				sendmail($adminemail,"","","Ticketing System : New Ticket Posted : ",$messagebody,"info@ticketingsystem.com");
	...
	public function update()
	{
		...
		else
		{
			...
			if(!empty($upd))
			{
				...
				if($state=="C")
				{
					sendmail($adminemail,"","","Ticketing System : Ticket Number ".$ticket_name." has been closed",$messagebody,"info@ticketingsystem.com");
					sendmail($customer,"","","Ticketing System : Ticket Number ".$ticket_name." has been closed",$messagebody,"info@ticketingsystem.com");
				}
				else
				{
					sendmail($adminemail,"","","Ticketing System : Ticket Number ".$ticket_name." has been modified",$messagebody,"info@ticketingsystem.com");
					sendmail($customer,"","","Ticketing System : Ticket Number ".$ticket_name." has been modified",$messagebody,"info@ticketingsystem.com");
				}
	...
	public function reply()
	{
		...
		$userdet=getUserInfoById($replierid);
		//echo "<pre>";
		//print_r($userdet);
		//echo "</pre>";
		$ticketid = $this->input->post('ticket_id');
		...
		{	
			if($data['replier'] == 'admin')
			{
				...
				$assigned=checkTicketAssignmentUser($data['ticket_id']);
				$assigneduserdet=getUserInfoById($assigned);
				$replieduserdet=getUserInfoById($data['replier_id']);
				
				$chkres=fetchmod('response_time');
				if($chkres==1)
				{
				...
				sendmail($repliedemailid,"","","Reply from the Ticketing System",$messagebody,"info@ticketingsystem.com");
			...
			else
			{
				...
				sendmail($assignedemailid,"","","Reply from the Ticketing System",$messagebody,"info@ticketingsystem.com");
				sendmail($adminemail,"","","Reply from the Ticketing System",$messagebody,"info@ticketingsystem.com");
