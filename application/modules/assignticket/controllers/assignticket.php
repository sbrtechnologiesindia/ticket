...
class assignticket extends MX_Controller {
	...
	public function ticketassign()
	{
		...
		$check = $this->assignticket_model->checkTicketAssignment($data['ticket_id']);
		if($check)
		{
			$r = $this->assignticket_model->assignTicket($data);
			if($r)
			{
				$user = getUserInfoById($data['admin_id'] );
				...
				sendmail($useremail,"","","Ticketing System : Ticket Number ".$ticket_name." has been assigned",$messagebody,"info@ticketingsystem.com");
		...
		else
		{
			...
			if(is_array($info))
			{
				$user = getUserInfoById($info[0]->admin_id);
				$ticket=$this->ticket_model->getticket($info[0]->ticket_id);
			}
			else
			{
				$user = getUserInfoById($info->admin_id);
	...
	public function tickettransfer()
	{
		...
		if(!$check)
		{
			...
			$user_old = getUserInfoById($info->admin_id);
			//$ticket=$this->ticket_model->getticket($info[0]->ticket_id);
			...
			if($r)
			{
				...
				sendmail($useremail,"","","Ticketing System : Ticket Number ".$ticket_name." has been transferred",$messagebody,"info@ticketingsystem.com");
