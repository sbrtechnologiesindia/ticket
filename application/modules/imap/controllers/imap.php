...
class imap extends MX_Controller {
	...
	public function getSubject($id)
	{
		if($this->imap_model->emailExists($id))
		{
			...
			$edata = $edata[0];
			$txt = (check_already_posted($edata->eid)) ? "<strong class='scanthreat'>[Posted]</strong>" : "";
		...
		else
		{
			...
			$txt = (check_already_posted($id)) ? "<strong class='scanthreat has-tip' title=\"Already posted as ticket\" >[P]</strong>" : "";
	...
	function getbody($id)
	{
		...
		if(!empty($parts))
		{
			...
			foreach ($parts as $part)
			{
				...
				if($chkopswat==1)
				{
					$scan = OPSWAT_scan_file(TICKET_PLUGIN_PATH."/tmp/".$filename);
					if(!empty($scan))
					{
