...
class theme extends MX_Controller {
	...
	public function insert()
	{
		...
		$this->form_validation->set_rules('theme_name', 'Theme Name', 'required|is_unique['.getDbPrefix().'theme.theme_name]');
		
		$this->form_validation->set_rules('theme_color', 'Theme Color', 'required|is_unique['.getDbPrefix().'theme.theme_color]|color_check');
		...
		else
		{
			...
			//code for dark color
			$themecolor_rgb=hex2rgb($theme_color);
			...
			$darktheme_color=rgb2hex($newthemecolorrgb);
			...
			$lighttheme_color=rgb2hex($newthemecolorrgb);
	...
	{
		$det=$this->theme_model->gettheme($companyid);
		$data['det']=$det;
