...
class backup extends MX_Controller {
	...
	{
		$data['backups'] = getFileList($this->path.'/backup/');
...
/*		echo '<pre>';
	...
	public function downloadbackup($file,$type=1)
	{
		...
		if($type == 2)
		{
			...
			$zfiles = getFileList(TICKET_PLUGIN_PATH.'/backup/');
		...
		if($type == 3)
		{
			...
			$zfiles = getFileList(TICKET_PLUGIN_PATH.'/CI/assets/attachments/',false);
	...
	public function restorebackup($name)
	{
		...
		$tables_array = preg_filter('/^/', getDbPrefix(), $tables);
