...
class statistics_model extends CI_Model
{
	...
	public function getRateChart($timeperiod,$customdatefrom,$customdateto)
	{
		if($timeperiod=="customdate")
		{
			$sql="select tick.ticket_no,count(rat.rating) as ratenum from ".$this->db->dbprefix."ticket tick,".$this->db->dbprefix."ticket_rating rat where tick.id=rat.ticket_id and date(rat.dateAdded) between '".date('Y-m-d',strtotime($customdatefrom))."' and '".date('Y-m-d',strtotime($customdateto))."'";
		}
		if($timeperiod=="today" or $timeperiod=="")
		{
			$sql="select tick.ticket_no,count(rat.rating) as ratenum from ".$this->db->dbprefix."ticket tick,".$this->db->dbprefix."ticket_rating rat where tick.id=rat.ticket_id and date(rat.dateAdded)='".date('Y-m-d')."'";
		}
		if($timeperiod=="yesterday")
		{
			$sql="select tick.ticket_no,count(rat.rating) as ratenum from ".$this->db->dbprefix."ticket tick,".$this->db->dbprefix."ticket_rating rat where tick.id=rat.ticket_id and date(rat.dateAdded)='".date('Y-m-d',strtotime("-1 day"))."'";
		}
		if($timeperiod=="lastweek")
		{
			$sql="select tick.ticket_no,count(rat.rating) as ratenum from ".$this->db->dbprefix."ticket tick,".$this->db->dbprefix."ticket_rating rat where tick.id=rat.ticket_id and date(rat.dateAdded)>='".date('Y-m-d',strtotime("-1 week"))."' and date(rat.dateAdded)<='".date('Y-m-d')."'";
		}
		if($timeperiod=="currentmonth")
		{
			$sql="select tick.ticket_no,count(rat.rating) as ratenum from ".$this->db->dbprefix."ticket tick,".$this->db->dbprefix."ticket_rating rat where tick.id=rat.ticket_id and MONTH(rat.dateAdded)='".date('m')."'";
		}
		if($timeperiod=="lastmonth")
		{
			$sql="select tick.ticket_no,count(rat.rating) as ratenum from ".$this->db->dbprefix."ticket tick,".$this->db->dbprefix."ticket_rating rat where tick.id=rat.ticket_id and MONTH(rat.dateAdded)='".date('m',strtotime("-1 month"))."'";
		}
		if($timeperiod=="currentyear")
		{
			$sql="select tick.ticket_no,count(rat.rating) as ratenum from ".$this->db->dbprefix."ticket tick,".$this->db->dbprefix."ticket_rating rat where tick.id=rat.ticket_id and YEAR(rat.dateAdded)='".date('Y')."'";
		}
		if($timeperiod=="lastyear")
		{
			$sql="select tick.ticket_no,count(rat.rating) as ratenum from ".$this->db->dbprefix."ticket tick,".$this->db->dbprefix."ticket_rating rat where tick.id=rat.ticket_id and YEAR(rat.dateAdded)='".date('Y',strtotime("-1 year"))."'";
		}
		$query=$this->db->query($sql);
		if(!empty($query))
		{
