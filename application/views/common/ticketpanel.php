...
<?php
$userdet=getUserInfoById($_SESSION['userid']);
//echo "<pre>";
//print_r($userdet);
//echo "</pre>";
$username=$userdet->username;
$responder_time_duration="";
$responder_time_duration=getresponsetimedur($_SESSION['userid']);
$repliedticketnumber=getticketreplied($_SESSION['userid']);
//echo "<pre>";
//print_r($responder_time_duration);
//echo "</pre>";
$chkrespond=fetchmod('response_time');
...
$averagerating=getticketrating();

$lowrate=getlowrate($_SESSION['userid']);
...
?>
	...
	if($chkrespond==1)
	{
		...
		if(!empty($responder_time_duration))
		{
		  ...
		  <?php
		  //}
		  echo $this->lang->line("Your current Response Time is")." ".smarttimestamp($responder_time_duration);
		}
		else
		{
	...
	<!-- @end #home-content -->
	  ...
	  {
	  ?>
          <p><?php echo $this->lang->line('Average Rating for the Tickets is');?> <?php echo $averagerating;?></p>
	  <?php
	  }
	  else
	  {
